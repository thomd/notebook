...
export function EditButton() {
  ...
  if (isReadPage(pathname)) {
    return (
      <>
        <Form action={`${pathname}edit`} className="inline">
          <button type="submit" className="mt-4 py-1 px-3 bg-gray-400 hover:bg-gray-500 text-white text-base rounded outline-none">
            Edit
          </button>
        </Form>
      </>
...
export function DeleteButton({ pageTitle }) {
  ...
  if (isReadPage(pathname)) {
    return (
      <>
        <button onClick={onOpen} className="py-1 px-3 bg-red-400 hover:bg-red-500 text-white text-base rounded outline-none">
          Delete
        </button>
